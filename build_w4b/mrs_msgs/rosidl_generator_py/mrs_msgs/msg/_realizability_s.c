...
bool mrs_msgs__msg__realizability__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("mrs_msgs.msg._realizability.Realizability", full_classname_dest, 41) == 0);
  }
  mrs_msgs__msg__Realizability * ros_message = _ros_message;
  {  // schema
  ...
  {  // rz_seq
    PyObject * field = PyObject_GetAttrString(_pymsg, "rz_seq");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->rz_seq = PyLong_AsUnsignedLongLong(field);
    Py_DECREF(field);
  }
  {  // entries
    ...
    if (!mrs_msgs__msg__RealizabilityEntry__Sequence__init(&(ros_message->entries), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create mrs_msgs__msg__RealizabilityEntry__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    mrs_msgs__msg__RealizabilityEntry * dest = ros_message->entries.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!mrs_msgs__msg__realizability_entry__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of Realizability */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("mrs_msgs.msg._realizability");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "Realizability");
  ...
  {  // rz_seq
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLongLong(ros_message->rz_seq);
    {
      int rc = PyObject_SetAttrString(_pymessage, "rz_seq", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
  ...
  {  // entries
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = mrs_msgs__msg__realizability_entry__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
