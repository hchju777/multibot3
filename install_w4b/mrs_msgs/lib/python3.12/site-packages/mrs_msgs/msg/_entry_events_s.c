...
bool mrs_msgs__msg__entry_events__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("mrs_msgs.msg._entry_events.EntryEvents", full_classname_dest, 38) == 0);
  }
  mrs_msgs__msg__EntryEvents * ros_message = _ros_message;
  {  // schema
  ...
  {  // events
    ...
    if (!mrs_msgs__msg__EntryEvent__Sequence__init(&(ros_message->events), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create mrs_msgs__msg__EntryEvent__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    mrs_msgs__msg__EntryEvent * dest = ros_message->events.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!mrs_msgs__msg__entry_event__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of EntryEvents */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("mrs_msgs.msg._entry_events");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "EntryEvents");
  ...
  {  // events
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = mrs_msgs__msg__entry_event__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
