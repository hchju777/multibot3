...
/// `mrs_msgs`, `mrs_trajopt`, `prototype` exist). The canon (`multibot3/CLAUDE.md`
/// CN-13~15) places `Status`/`Result<T,E>`/`ContractViolation` in `mrs_core`.
/// Until that package exists we define the vocabulary locally, in the
/// `mrs_mapf::core` namespace, so the pure core builds without ROS. When
/// `mrs_core` lands, this header is replaced by an include of
/// `mrs_core/status.hpp` and the local definitions are deleted. Names and
/// semantics are kept identical to `mrs_trajopt::core` (same precedent) so the
/// swap is textual. See `20b_impl_mapf.md` "알려진 한계".
...
/// declaration instead. `run_once()` (service/) never throws this; it returns
/// `Status`/`Result` on the normal path and reserves this type for malformed
