...
bool mrs_msgs__msg__robot_trajectory__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("mrs_msgs.msg._robot_trajectory.RobotTrajectory", full_classname_dest, 46) == 0);
  }
  mrs_msgs__msg__RobotTrajectory * ros_message = _ros_message;
  {  // robot
  ...
  {  // points
    ...
    if (!mrs_msgs__msg__TrajectoryPoint__Sequence__init(&(ros_message->points), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create mrs_msgs__msg__TrajectoryPoint__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    mrs_msgs__msg__TrajectoryPoint * dest = ros_message->points.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!mrs_msgs__msg__trajectory_point__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * mrs_msgs__msg__robot_trajectory__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("mrs_msgs.msg._robot_trajectory");
  ...
  {  // points
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = mrs_msgs__msg__trajectory_point__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
