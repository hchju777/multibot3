...
/**
 * @file view_ids.hpp
 * @brief 뷰별 노드 id 강타입과 뷰 스코프 — 계약 §0.1 규칙 V1~V4 의 C++ 구현.
 *
 ...
 *
 * 세 뷰의 노드 id 를 전부 맨 `std::uint32_t` 로 두면 **잘못된 뷰의 id 를 넘겨도 컴파일된다**.
 ...
 * CI 경계 검사(`tools/check_boundaries.sh`)가 맡는다.
 *
 * @note 이 헤더는 `mrs_core_types` 에 놓이며 **rclcpp·mrs_interfaces 에 의존하지 않는다**
 *       (아키텍처 제약 C1).
 * @note **엣지 id 는 아직 강타입이 아니다.** 계약 §0.1 정본표가 엣지에도 뷰를 고정하지만
 *       (`violating_physical_edge_ids` vs `violating_uniform_edge_ids`), 이번 재잠금 범위는
 *       노드 3종이다. 엣지 강타입화는 [1](D-11 MapRegistry) 착수 시 재판정한다.
 */
...
/**
 * @brief 노드 id 가 속한 뷰의 **종류**. `ViewScope.msg` 의 `VIEW_KIND_*` 상수와 값이 일치한다.
 *
...
/**
 * @brief 뷰 종류를 컴파일 시각에 고정한 노드 식별자 (phantom tag 강타입).
 *
 * `std::uint32_t` 와도, 다른 뷰의 `ViewNodeId` 와도 **암묵 변환되지 않는다**. 크기와 표현은
 * `std::uint32_t` 와 같으므로 직렬화·메모리 비용이 늘지 않는다(파일 하단 `static_assert` 로 보장).
 *
...
public:
   ...
   * @note 기본 생성은 항상 센티넬이다 — 0 은 유효한 노드 id 이므로 기본값으로 쓰면 안 된다.
   */
  constexpr ViewNodeId() noexcept = default;
  ...
  /**
   * @brief 원시 값을 꺼낸다.
   * @return `std::uint32_t` — 뷰 @p K 안에서의 노드 번호. 센티넬이면 4294967295.
   * @warning 이 값을 다른 뷰의 `ViewNodeId` 생성자에 그대로 넣지 말 것. 뷰 간 변환은
   *          MapRegistry 의 `transform_node` 만이 수행한다(C4).
   */
   ...
   * @return `bool` — 센티넬(4294967295)이면 true, 유효한 노드 id 이면 false.
   */
   ...
   * @return `ViewNodeId<K>` — 센티넬 값을 담은 id.
   */
  [[nodiscard]] static constexpr ViewNodeId none() noexcept
  {
    return ViewNodeId{};
  }

  /**
   * @brief 두 id 가 같은지 비교한다. 뷰가 다르면 애초에 컴파일되지 않는다.
   * @param[in] lhs 좌변 id. 자료형 `ViewNodeId<K>`.
   * @param[in] rhs 우변 id. 자료형 `ViewNodeId<K>`.
   * @return `bool` — 원시 값이 같으면 true.
   */
  friend constexpr bool operator==(ViewNodeId lhs, ViewNodeId rhs) noexcept
  {
...
private:
  /** @brief "해당 없음"을 뜻하는 원시 센티넬 값. 전 뷰 공통(계약 §0 전역 규약). */
...
/**
 * @brief 노드 id 가 유효한 뷰의 **인스턴스**까지 식별하는 스코프 (`ViewScope.msg` 미러, 규칙 V2).
 *
...
/**
 * @brief `mrs::ViewNodeId` 의 해시 특수화 — `std::unordered_map`·`unordered_set` 키로 쓰기 위한 것.
 *
 * 뷰 종류별로 서로 다른 타입이므로 이 부분 특수화 하나가 3종 전부를 덮는다.
 *
 * @tparam K 뷰 종류.
 */
template <mrs::ViewKind K> struct hash<mrs::ViewNodeId<K>>
{
   ...
   * @param[in] id 해시할 id. 자료형 `mrs::ViewNodeId<K>`.
   * @return `std::size_t` — 원시 값의 해시. 뷰가 달라도 같은 원시 값이면 같은 해시이나,
   *         타입이 달라 서로 다른 컨테이너에만 들어가므로 충돌 문제가 되지 않는다.
   */
  [[nodiscard]] size_t operator()(mrs::ViewNodeId<K> id) const noexcept
  {
