...
///
/// 🔴 TODO(integration): the `mrs_core` package is absent in this checkout (only
/// `mrs_msgs`, the four algorithm packages and `prototype` exist). The canon
/// (`multibot3/CLAUDE.md` CN-13~15) places `Status`/`Result<T,E>`/
/// `ContractViolation` in `mrs_core`. Until that package exists we define the
/// vocabulary locally, in the `mrs_sadg::core` namespace, so the pure core
/// builds without ROS. When `mrs_core` lands, this header is replaced by an
/// include of `mrs_core/status.hpp` and the local definitions are deleted. Names
/// and semantics are kept identical to `mrs_mapf::core` / `mrs_trajopt::core`
/// (same precedent) so the swap is textual. See `20c_impl_sadg.md` "알려진 한계".
...
///
/// Never thrown inside a control-period tick / planning tick — CN-16 requires
/// tick-path violations to become a fatal latch + counter + safe-stop
/// declaration instead. `ExecutionTickService::run_tick()` never throws this; it
/// returns its outcome on the normal path and reserves this type for malformed
/// static configuration discovered at startup (e.g. plugin registry empty).
