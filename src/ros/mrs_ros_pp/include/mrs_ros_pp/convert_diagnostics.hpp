 ...
 * "왜 폐기됐는가"를 복원할 수 없고, 그것이 F2 발동률·F6 스래싱 계측 오염과 같은 형태의 손실이다.
 * 그래서 `ConvertStatus` 열거 전부에 칸을 하나씩 준다.
 *
 * ## 함수 지역 `static` 을 쓰지 않는 이유
 * Nav2 참조(§D2)가 명시 금지했다 — 컴포지션으로 노드 ×N 이 한 프로세스에 뜨면 카운터가 공유되어
 * 로봇별·노드별 분해가 불가능해진다. 그래서 **노드 인스턴스가 소유하는 값 객체**로 둔다.
 *
 * @note ⚠ 이 헤더는 `mrs_ros_sadg` 의 동명 헤더와 내용이 같다. [0a] 시점에 `ros/` 공통 패키지가
 *       없어 패키지별로 사본을 둔 것이며, **공통 `ros/` 헬퍼 패키지로 승격할 대상**이다
 *       (nav2-reference §A1 이 `NodeThread` 상당 래퍼에 대해 지시한 것과 같은 처분).
 */
 ...
 * @note 계약 §0.2 표에 사유가 추가되면 이 값도 늘어난다 — 미러의 미러이므로 값을 손으로
 *       늘리지 말고 열거 마지막 값을 따라 갱신할 것.
 */
 ...
 *
 * 예외를 던지지 않으며 할당도 하지 않는다 — 실시간 경로(릴리스·발행 콜백)에서 호출되기 때문이다.
 */
